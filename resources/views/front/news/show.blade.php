          ...
          <div class="body-content lg:w-[80%] max-lg:w-full overflow-hidden px-1">
            ...
            <div class="btns flex w-full justify-between items-center sm:mt-2 max-sm:mt-4 flex-wrap">
              ...
              <div class="other-btns md:w-fit flex justify-between items-center max-md:w-full max-md:-translate-y-6">
                 @if (isset($new->previous))
                <a href="{{ route('new.show', $new->previous->{'slug_' . app()->getLocale()}) }}" class="sm:py-2 sm:px-3 max-sm:py-1 max-sm:px-2 uppercase text-dimgray sm:text-[14px] text-center max-sm:text-[12px] border border-[#E0E0E0] mr-2">
                  @lang('main.oldingi_yangiliklar')
                </a>
                @endif
                @if (isset($new->next))
                <a href="{{ route('new.show', $new->next->{'slug_' . app()->getLocale()}) }}" class="sm:py-2 sm:px-3 max-sm:py-1 max-sm:px-2 uppercase text-dimgray sm:text-[14px]  text-center max-sm:text-[12px] border border-[#E0E0E0]">
